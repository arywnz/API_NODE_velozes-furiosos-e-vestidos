...
const Motos = () => {
  ...
  const carregarMotos = async () => {
    setLoading(true);
    try {
      const data = await api.motos.listar(token);
      setMotos(data);
    } catch (err) {
      setError(err.message || 'Falha ao buscar motos');
    } finally {
      setLoading(false);
    }
  };
  ...
  const salvar = async (e) => {
    ...
    try {
      if (editandoId) {
        await api.motos.atualizar(editandoId, dados, token);
        setSuccess('Moto atualizada com sucesso!');
      } else {
        await api.motos.criar(dados, token);
        setSuccess('Moto cadastrada com sucesso!');
      }
      setModalAberto(false);
      carregarMotos();
    } catch (err) {
      setError(err.message || 'Falha ao salvar moto');
    }
  };

  const deletar = async (id) => {
    if (!window.confirm('Deseja realmente excluir esta moto?')) return;
    setError('');
    setSuccess('');

    try {
      await api.motos.excluir(id, token);
      setSuccess('Moto removida com sucesso!');
      carregarMotos();
    } catch (err) {
      setError(err.message || 'Erro ao remover moto');
    }
  };
    ...
    <>
      <div className="space-y-6 transicao-pagina">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div>
            <h2 className="text-2xl font-bold tracking-tight text-white">Motos (NoSQL)</h2>
            <p className="text-sm text-zinc-400 mt-1">Gerenciamento de motocicletas no banco não-relacional MongoDB.</p>
          </div>
          <button
            onClick={abrirCriar}
              ...
              <table className="w-full text-left border-collapse text-sm">
                <thead>
                  <tr className="bg-zinc-800/30 border-b border-zinc-800">
                    ...
                    <th className="px-5 py-3.5 text-zinc-400 font-bold uppercase tracking-wider text-xs">Cilindrada (cc)</th>
                ...
                <tbody className="divide-y divide-zinc-800/80">
                  {motos.map((m) => (
                    <tr key={m._id} className="hover:bg-zinc-800/10 transition-colors">
                      <td className="px-5 py-3.5 text-white font-semibold">{m.marca}</td>
                      <td className="px-5 py-3.5 text-zinc-300">{m.modelo}</td>
                      <td className="px-5 py-3.5 text-zinc-300">{m.ano}</td>
                      <td className="px-5 py-3.5 text-zinc-300">{m.cilindrada}</td>
                      <td className="px-5 py-3.5 text-amber-500 font-semibold">
                        {new Intl.NumberFormat('pt-BR', { style: 'currency', currency: 'BRL' }).format(m.preco)}
                      </td>
          ...
          <div className="w-full max-w-md bg-zinc-900 border border-zinc-800 rounded-xl shadow-2xl p-6 relative max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={salvar} className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                ...
                <div>
                  <label className="block text-xs font-bold text-zinc-400 uppercase tracking-wider mb-1.5">Cilindrada (cc)</label>
                  <input
                    type="number"
                    required
                    value={cilindrada}

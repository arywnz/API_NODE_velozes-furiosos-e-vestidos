...
DesenvolVi a aplicação em Node.js com Express como foi solicitado no enunciado da prova e trabalho e implementeo duas camadas de banco de dados para persistência, alinhada com boas práticas de segurança baseadas no OWASP Top 10, totalmente testada e conteinerizada com Docker.

## Tecnologias e Decisões Adotadas

1. **Backend com Node.js e Express**: Escolha  eficiente para construção de APIs assíncronas de alto desempenho.
...
5. **Swagger**: Geração automática da documentação da API em `/api-docs`.
...
7. **Conteinerização com Docker e Docker Compose**: Toda a aplicação e os bancos de dados funcionam no Docker Compose.

## Como Executar a Aplicação via Docker

Ter o docker instalado na maquina e rodando.
